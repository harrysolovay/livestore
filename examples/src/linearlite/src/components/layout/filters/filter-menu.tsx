  ...
  return (
    <MenuTrigger>
      ...
      <Popover className="w-48 bg-white dark:bg-neutral-800 rounded-lg shadow-md border border-neutral-200 dark:border-neutral-700 text-sm leading-none">
        <Menu className="focus:outline-hidden" selectionMode="multiple">
          {type !== 'priority' && (
              ...
              {statusOptions.map(({ name, icon, style }, statusOption) => {
                ...
                return (
                  <MenuItem
                    ...
                    className="group/item p-2 pl-9 rounded-md hover:bg-neutral-100 dark:hover:bg-neutral-700 focus:outline-hidden focus:bg-neutral-100 dark:focus:bg-neutral-700 cursor-pointer flex items-center gap-2"
                  >
                    <div
                      className={`absolute left-4 size-4 rounded-sm ${active ? 'bg-indigo-500' : 'hidden group-hover/item:block group-focus/item:block border border-neutral-300 dark:border-neutral-600'}`}
                    >
