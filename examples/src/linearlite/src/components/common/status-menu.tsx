  ...
  return (
    <MenuTrigger isOpen={isOpen} onOpenChange={setIsOpen}>
      <Button
        ...
        className="group h-8 min-w-8 rounded-lg flex gap-1.5 px-2 items-center justify-center hover:bg-neutral-100 dark:hover:bg-neutral-800 focus:outline-hidden focus:bg-neutral-100 dark:focus:bg-neutral-800"
      >
      ...
      >
        <Menu className="focus:outline-hidden" {...keyboardProps}>
          {statusOptions.map(({ name, icon, style, shortcut }, statusOption) => (
            <MenuItem
              key={statusOption}
              onAction={() => onStatusChange(statusOption as Status)}
              className="p-2 rounded-md hover:bg-neutral-100 dark:hover:bg-neutral-700 focus:outline-hidden focus:bg-neutral-100 dark:focus:bg-neutral-700 cursor-pointer flex items-center gap-2"
            >
